...
export default React.memo(function (props) {
    ...
    var content;
    if (type === 'input') {
        content = React.createElement(Input, { size: "small", placeholder: "\u8BF7\u8F93\u5165", value: innerState.value, onChange: handleInputChange });
    }
    else if (type === 'inputWithUnit') {
        content = React.createElement(Input, { size: "small", placeholder: "\u8BF7\u8F93\u5165", value: innerState.value, onChange: handleInputChange, onBlur: function () { callback(innerState); }, onPressEnter: function () { callback(innerState); }, addonAfter: React.createElement(Select, { size: "small", value: innerState.unit, onChange: handleChange.bind(_this, 'unit') }, units.map(function (item) { return React.createElement(Option, { key: item, value: item }, item); })) });
    }
    else if (type === 'select') {
        content = React.createElement(Select, { size: "small", style: { minWidth: '100%' }, placeholder: "\u8BF7\u9009\u62E9", allowClear: allowClear, value: innerState.value === '' ? undefined : innerState.value, onChange: handleChange.bind(_this, 'value') }, options.map(function (item) { return React.createElement(Option, { key: item, value: item }, item); }));
    }
    else if (type === 'colorPicker') {
        content = React.createElement(Input, { size: "small", placeholder: "\u8BF7\u8F93\u5165", value: innerState.value, onChange: handleInputChange, addonAfter: React.createElement(Dropdown, { visible: colorPickerVisible, overlay: React.createElement(SketchPicker, { color: innerState.value, onChangeComplete: handleColorChange }) },
